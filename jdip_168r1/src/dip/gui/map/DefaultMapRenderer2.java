		...
		if(provinceGroupElement != null)
		{
			...
			else
			{
				...
				else
				{
					...
					// CRJ: Copied from influence mode above to add highlighting to all provinces
					// based on last occupier, not just Supply Centers
					if(position.getLastOccupier(province) != null && province.isLand() && !province.hasSupplyCenter())
					{
						setCSSIfChanged(provinceGroupElement, 
							tracker.getPowerCSSClass(position.getLastOccupier(province)));
					}
